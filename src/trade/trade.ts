...
export function attachTradeMethods(TradeClass: new (...args: any[]) => any) {
    ...
    TradeClass.prototype.buyUpToken = async function (): Promise<void> {
        ...
        try {
            ...
            console.log(`⚠️ Buy up token | ${kind}: ${why}`);
            ...
            if (err?.status === 401 || (typeof err?.data?.error === "string" && err.data.error.includes("Unauthorized"))) {
                console.log("💡 Check signer, funder, signature type, and derived CLOB API credentials.");
            }
        } finally {
    ...
    TradeClass.prototype.buyDownToken = async function (): Promise<void> {
        ...
        try {
            ...
            const { kind, why } = describeTradingError(error);
            const cooldownSec = globalThis.__CONFIG__.entry_buy_cooldown_sec;
            this.entryBuyCooldownUntil = Date.now() + cooldownSec * 1000;
            console.log(`⚠️ Buy down token | ${kind}: ${why}`);
            console.log(
                `⏳ Entry buy paused ${cooldownSec}s; will retry when conditions match again.`
            );
            const err = error as { status?: number; data?: { error?: string } };
            if (err?.status === 401 || (typeof err?.data?.error === "string" && err.data.error.includes("Unauthorized"))) {
                console.log("💡 Check signer, funder, signature type, and derived CLOB API credentials.");
    ...
    TradeClass.prototype.sellUpToken = async function (): Promise<boolean> {
        ...
        try {
            ...
            const { kind, why } = describeTradingError(error);
            console.log(`⚠️ Sell up token | ${kind}: ${why}`);
            const err = error as { status?: number; data?: { error?: string } };
            if (err?.status === 401 || (typeof err?.data?.error === "string" && err.data.error.includes("Unauthorized"))) {
                console.log("💡 Check signer, funder, signature type, and derived CLOB API credentials.");
            }
    ...
    TradeClass.prototype.sellDownToken = async function (): Promise<boolean> {
        ...
        try {
            ...
            const { kind, why } = describeTradingError(error);
            console.log(`⚠️ Sell down token | ${kind}: ${why}`);
            const err = error as { status?: number; data?: { error?: string } };
            if (err?.status === 401 || (typeof err?.data?.error === "string" && err.data.error.includes("Unauthorized"))) {
                console.log("💡 Check signer, funder, signature type, and derived CLOB API credentials.");
            }
